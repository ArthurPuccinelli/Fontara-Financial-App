...
document.addEventListener('DOMContentLoaded', async function() {
  console.log("assinatura-embarcada.js (vFinal-Rev2 - Debugging .on()): Script carregado.");
  ...
  async function initializeDocuSignSdk() {
    ...
    try {
      ...
      if (DOCUSIGN_APP_CLIENT_ID) {
        ...
        console.log("[assinatura-embarcada.js] window.DocuSign.loadDocuSign completado. Retorno:", loadedInstance); // ADICIONADO LOG DO loadedInstance
        
        if (loadedInstance && typeof loadedInstance.signing === 'function') {
          docusignApi = loadedInstance;  //
          sdkInitializationSuccessful = true; //
          console.log("[assinatura-embarcada.js] SDK DocuSign (retornado por loadDocuSign) está pronto. Objeto docusignApi:", docusignApi); //
        } else {
  ...
  function openDocusignSigningModal(url, signingMode) {
    ...
    if (signingMode === 'classic') {
        ...
        try {
            ...
            if (!currentSigningInstance || typeof currentSigningInstance.mount !== 'function') { 
                console.error("[assinatura-embarcada.js] currentSigningInstance.mount não é uma função ou é inválido:", currentSigningInstance); //
                throw new Error("Falha ao criar instância de assinatura DocuSign com método mount válido."); //
            }
            ...
            if (currentSigningInstance && typeof currentSigningInstance.on === 'function') {
                console.log("[assinatura-embarcada.js] currentSigningInstance AINDA É VÁLIDO e .on é uma função ANTES de anexar listeners."); //
                ...
                currentSigningInstance.on('sessionEnd', (eventData) => {
                 ...
                 // Adicione outros listeners aqui se necessário, com logs similares
            } else {
                 console.error("[assinatura-embarcada.js] currentSigningInstance INVÁLIDO ou .on não é função IMEDIATAMENTE ANTES de anexar listeners. CurrentSigningInstance:", currentSigningInstance); //
                 // Se cair aqui, é um forte indicador de que a instância foi corrompida ou não é o que se espera
            }
            // --- FIM DEBUGGING LOG ---
