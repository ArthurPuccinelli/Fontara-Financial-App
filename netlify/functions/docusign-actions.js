...
async function getAuthenticatedApiClient() {
  ...
  console.log("[docusign-actions] getAuthenticatedApiClient: Verificando variáveis de ambiente...");
  if (!ik || !userId || !rsaPrivateKeyBase64Encoded || !authServer || !basePath || !accountId) {
    ...
    throw new Error(errorMessage); // Será capturado pelo handler principal
  }
  ...
  try {
    ...
    if (!rsaPrivateKeyPemString.startsWith("-----BEGIN RSA PRIVATE KEY-----") || !rsaPrivateKeyPemString.endsWith("-----END RSA PRIVATE KEY-----")) {
      console.error("[docusign-actions] Chave PEM decodificada de Base64 está mal formatada (delimitadores).");
  ...
  try {
    console.log(`[docusign-actions] Tentando autenticação JWT...`);
...
// Função auxiliar para logar erros da API Docusign e construir mensagem
function formatDocusignError(actionName, errorObject) {
    console.error(`--------------------------------------------------------------------`);
    ...
    let docusignSpecificDetail = "Detalhes específicos do Docusign não capturados ou não presentes na resposta.";
    if (errorObject.response) {
        ...
        if (errorBody) {
            ...
            if (typeof errorBody === 'object' && errorBody !== null) {
                docusignSpecificDetail = errorBody.message || errorBody.error_description || errorBody.error || JSON.stringify(errorBody);
                if (errorBody.errorDetails && Array.isArray(errorBody.errorDetails) && errorBody.errorDetails.length > 0) {
                    ...
                    docusignSpecificDetail += ` | Detalhes Adicionais: ${detailsMessages}`;
                } else if (errorBody.errorCode && errorBody.message && !docusignSpecificDetail.includes(errorBody.message)) {
                     docusignSpecificDetail = `Código: ${errorBody.errorCode}, Mensagem: ${errorBody.message}`;
                }
            } else if (typeof errorBody === 'string') {
                docusignSpecificDetail = errorBody;
            }
        } else {
...
async function createEnvelopeForEmbeddedSigning(apiClient, envelopeArgs) {
  ...
  try {
    ...
    const docusignErrorMessage = formatDocusignError("createEnvelopeForEmbeddedSigning (template)", err);
...
async function createDynamicEnvelope(apiClient, envelopeArgs) {
  ...
  env.documents = envelopeArgs.documents.map(doc => 
    docusign.Document.constructFromObject({
      documentBase64: doc.documentBase64, name: doc.name,
      fileExtension: doc.fileExtension, documentId: doc.documentId,
      transformPdfFields: "false" // Adicionado na última vez
    })
  );
  ...
  env.recipients = docusign.Recipients.constructFromObject({
    ...
    carbonCopies: (envelopeArgs.recipients.carbonCopies || []).map(cc => 
        docusign.CarbonCopy.constructFromObject({ email: cc.email, name: cc.name, recipientId: String(cc.recipientId), routingOrder: String(cc.routingOrder || (envelopeArgs.recipients.signers.length + 1)) })
    )
  });
  env.status = envelopeArgs.status || "sent";

  console.log("[docusign-actions] Criando envelope dinâmico. Definição (sem base64 do doc):", JSON.stringify({ ...env, documents: env.documents.map(d => ({...d, documentBase64: "BASE64_REMOVIDA_DO_LOG"})) }, null, 2));
  try {
    const results = await envelopesApi.createEnvelope(accountId, { envelopeDefinition: env });
    console.log("[docusign-actions] Envelope dinâmico criado com sucesso. ID:", results.envelopeId);
    return results.envelopeId;
  } catch (err) {
    const docusignErrorMessage = formatDocusignError("createDynamicEnvelope", err);
...
async function createRecipientView(apiClient, args) {
  ...
  let viewRequestOptions = {
    returnUrl: args.returnUrl, authenticationMethod: 'none', 
    email: args.signerEmail, userName: args.signerName, clientUserId: args.signerClientUserId,
  };
  if (args.useFocusedView === true) {
    viewRequestOptions.chromeControls = 'hide';
  } else {
    viewRequestOptions.chromeControls = 'show';
  }
  ...
  try {
    ...
    const docusignErrorMessage = formatDocusignError("createRecipientView", err);
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (typeof event.body === "string") {
      ...
      throw new Error("Corpo da requisição vazio ou em formato inesperado.");
    }
  ...
  try {
    ...
    return { statusCode: 200, body: JSON.stringify(resultData) }; // Retorna JSON válido

  } catch (error) {
    console.error(`[docusign-actions] ERRO FINAL NO HANDLER para ação '${action}':`, error.message);
    // O error.message já deve conter os detalhes do Docusign devido à formatação nas funções internas
    return { 
        statusCode: 500, 
        body: JSON.stringify({ 
        ...
        headers: { 'Content-Type': 'application/json' } // Garante que o erro também seja JSON
    };
  }
};
